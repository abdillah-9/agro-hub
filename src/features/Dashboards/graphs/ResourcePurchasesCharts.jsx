...
export default function ResourcePurchasesCharts({ stats }) {
  ...
  const COLORS = ["#ff6b6b", "#ffe66d", "#4ecdc4"]; // New pie colors
  const BAR_COLORS = { items: "#ff6b6b", total: "#4ecdc4" }; // Bar colors

  return (
    <div style={{ width: "100%", marginTop: "20px" }}>
      {/* MAIN HEADING */}
      <div style={{ textAlign: "center", maxWidth: "500px", margin: "0 auto", marginBottom: 30 }}>
        <div style={{ fontSize: 16, fontWeight: 600 }}>Resource Purchases Statistics</div>
        <div style={{ fontSize: 12, color: "#718096" }}>
          Pending, shipped, and completed resource purchases
        </div>
      </div>
        ...
        <div style={{ width: "100%", maxWidth: "500px", marginBottom: 40 }}>
          ...
          <ResponsiveContainer width="100%" height={350}>
            <BarChart data={barData} barCategoryGap={10} barGap={0}>
              ...
              <Bar yAxisId="left" dataKey="items" fill={BAR_COLORS.items} name="Items Purchased" barSize={15} />
              <Bar yAxisId="right" dataKey="total" fill={BAR_COLORS.total} name="Total Cost (Tsh)" barSize={15} />
            </BarChart>
          </ResponsiveContainer>
        </div>
