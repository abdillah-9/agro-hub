...
export default function ResourceSalesCharts({ stats }) {
  ...
  const COLORS = ["#4dabf5", "#ffa600", "#2e7d32"]; // Colors for pie slices

  return (
    <div style={{ width: "100%", marginTop: "20px" }}>
      {/* MAIN HEADING */}
      <div style={{ textAlign: "center", maxWidth: "500px", margin: "0 auto", marginBottom: 30 }}>
        <div style={{ fontSize: 16, fontWeight: 600 }}>Resource Sales Statistics</div>
        <div style={{ fontSize: 12, color: "#718096" }}>
          Pending, shipped, and completed resource sales
        </div>
      </div>
          ...
          <ResponsiveContainer width="100%" height={350}>
            <BarChart data={barData} barCategoryGap={10} barGap={0}>
              ...
              <YAxis
                yAxisId="left"
                tick={{ fontSize: "10px" }}
                label={{ value: "Items Sold", angle: -90, position: "insideLeft", fontSize: "12px" }}
                width={35}
              />
              <YAxis
                yAxisId="right"
              ...
              <Bar yAxisId="left" dataKey="items" fill="#4dabf5" name="Items Sold" barSize={15} />
              <Bar yAxisId="right" dataKey="total" fill="#2e7d32" name="Total (Tsh)" barSize={15} />
            </BarChart>
          </ResponsiveContainer>
        </div>
